...
int main(int argc, char *argv[]) {
    ...
    bytes_send = write(sockfd, (char *)&exit_msg, DESTINATION_SIZE + SOURCE_SIZE + TYPE_SIZE);
    ...
    printf("Sending the rest of the exit message!\n");
    write(sockfd, (char *)&exit_msg + bytes_send, HEADER_SIZE-bytes_send);

    close(sockfd);

    // Lets do the same with the error message!
}
