...
message create_chat_message(char *clientID) {
    ...
    strncpy(chat_msg.header.source, clientID, SOURCE_LENGTH);
    ...
    strncpy(chat_msg.header.destination, "Edu0210", SOURCE_LENGTH);
    chat_msg.header.Message_ID = (uint32_t) htonl(1);



    char *test_message = "Hello there this is my message!";
    chat_msg.header.Length = (uint32_t) htonl(strlen(test_message) + 1);
    memcpy(chat_msg.content, test_message, strlen(test_message) + 1);
...
void handle_message_from_server(char *buffer, short int msg_type) {
    ...
    case ERROR_CANNOT_DELIVER:
        message error_msg_alr;
        // store the first 50 bytes into the header
        memcpy((char *) &error_msg_alr.header, buffer, HEADER_SIZE);
        error_msg_alr.header.Type = ntohs(error_msg_alr.header.Type);
        error_msg_alr.header.Length = ntohl(error_msg_alr.header.Length);
        error_msg_alr.header.Message_ID = ntohl(error_msg_alr.header.Message_ID);
        printf("**************************\n");
        printf("Received Hello ACK\n");
        printf("Info about header: \n");
        printf("Header Type: %hd\n", error_msg_alr.header.Type);
        printf("Header MessageID: %u\n", error_msg_alr.header.Message_ID);
        printf("Header Length: %hd\n", error_msg_alr.header.Length);
        printf("ClientID: %s\n", error_msg_alr.header.source);
        printf("Destination: %s\n", error_msg_alr.header.destination);
        printf("**************************\n");
        break;
    case CHAT:
