...
int write_message_to_client_bad(message *m, int fd, int size)
{
    int len = m->h.length;
    int write_size = HDR_SIZE + len;
    ...
    // NOTE: This cannot be blocking. If this blocks we have a big problem
    int n = write(fd, buf, size + HDR_SIZE);
...
int send_message(char *name, int fd)
{
    ...
    // For testing purposes only
        ...
        if (fgets(source, sizeof(source), stdin) != NULL) {
            strip_newline(source, sizeof(source));
        }
        printf("Source: %s\n\n", source);
        ...
        if (fgets(dest, sizeof(dest), stdin) != NULL) {
            strip_newline(dest, sizeof(dest));
        }
        ...
        if (fgets(len_str, sizeof(len_str), stdin) != NULL) {
            strip_newline(len_str, sizeof(len_str));
        }
        ...
        if (fgets(msgid_str, sizeof(msgid_str), stdin) != NULL) {
            strip_newline(msgid_str, sizeof(msgid_str));
        }
        int msg_id = atoi(msgid_str);
        printf("Message ID: %d\n\n", msg_id);

        // Create the message
        message *m = new_message(1, source, dest, length, msg_id, NULL);
        write_message_to_client(m, fd);
        free_message(&m);
    }

    else if (strcmp("BADLISTREQ", buf) == 0) {
        char source[50];
        char dest[50];
        char len_str[10];
        char msgid_str[10];
        ...
        // Create the message with type 3 instead of type 1
        message *m = new_message(3, source, dest, length, msg_id, NULL);
        write_message_to_client(m, fd);
        free_message(&m);
    }


    else if (strcmp("MSGTOOBIG", buf) == 0) {
        printf("Who are you sending a message to?\n");
        memset(buf, 0, 20);
        ...
        printf("What is the message (make bigger than 10 bytes?)\n");
        char bigbuf[100];
        memset(bigbuf, 0, 100);
        ...
        message *m = new_message(5, name, buf, 10, 1, bigbuf);
        write_message_to_client(m, fd);
        free_message(&m);
    ...
    else if (strcmp("MSGTOOSMALL", buf) == 0) {
        printf("Who are you sending a message to?\n");
        memset(buf, 0, 20);
        ...
        printf("What is the message (make smaller than 100 bytes?\n");
        char bigbuf[100];
        memset(bigbuf, 0, 100);
        ...
        message *m = new_message(5, name, buf, 200, 1, bigbuf);
        write_message_to_client_bad(m, fd, 100);
        free_message(&m);
    }

    else if (strcmp("SLEEP", buf) == 0) {
        sleep(1);
    }

    else if (strcmp("HELLOACK", buf) == 0) {
        // HELLO ACK
        message *m = new_message(2, name, "SERVER", 0, 0, NULL);
        write_message_to_client(m, fd);
        free_message(&m);
    }

    else if (strcmp("CLIENTLIST", buf) == 0) {
        // CLIENT LIST
        message *m = new_message(4, name, "SERVER", 0, 0, NULL);
        write_message_to_client(m, fd);
        free_message(&m);
    }

    else if (strcmp("ERRORPRESENT", buf) == 0) {
        // ERROR client already present
        message *m = new_message(7, name, "SERVER", 0, 0, NULL);
        write_message_to_client(m, fd);
        free_message(&m);
    }

    else if (strcmp("ERRORDELIVER", buf) == 0) {
        // ERROR cannot deliver
        message *m = new_message(8, name, "SERVER", 0, 0, NULL);
        write_message_to_client(m, fd);
        free_message(&m);
    }

    else if (strcmp("MEGATYPE", buf) == 0) {
        // ERROR cannot deliver
        message *m = new_message(100, name, "SERVER", 0, 0, NULL);
        write_message_to_client(m, fd);
        free_message(&m);
    }

    if (strcmp("BUILDMSG", buf) == 0) {
        printf("Recognized command BUILDMSG.\n");
        ...
        // Get the type input
        printf("Enter the type (as a number):\n");
        memset(type_str, 0, sizeof(type_str));
        if (fgets(type_str, sizeof(type_str), stdin) != NULL) {
            strip_newline(type_str, sizeof(type_str));
        }
        ...
        // Validate length to ensure it's within limits
        if (length < 1 || length > 400) {
            printf("Error: Invalid message length. Must be between 1 and 400.\n");
            return 0;  // Exit the command
        }

        // Get the message ID input
        printf("Enter the message ID (as a number):\n");
        memset(msgid_str, 0, sizeof(msgid_str));
        if (fgets(msgid_str, sizeof(msgid_str), stdin) != NULL) {
            strip_newline(msgid_str, sizeof(msgid_str));
        }
        ...
        // Ensure the message length matches the provided length
        if (strlen(bigbuf) > length) {
            printf("Warning: Message length exceeds the specified length.\n");
        }

        // Create the message
        message *m = new_message(type, source, dest, length, msg_id, bigbuf);
        write_message_to_client(m, fd);
        free_message(&m);
    }
